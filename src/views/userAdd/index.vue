<template>
  ...
  >
      ...
      <div class="content" :style="{ minHeight: this.pageHeight }">
        <div class="content_wrap">
          ...
          <div class="info_list">
            <dof-cell
              title="昵称"
              :right-text="userInfo.username || '请输入'"
    ...
    >
      ...
      <div slot="middle">
        <text class="title_text">添加新的成员</text>
      </div>
    </dof-minibar>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    ...mapActions(['addScaleUser', 'getBaseInfo', 'uploadPicToOss']),
    getScrollY(e) {
    ...
    async save() {
      ...
      const res = await this.addScaleUser(this.userInfo)
      if (res.isSuccess) {
        debugUtil.log('添加成功', res)
        this.$storage.setStorage(
          `${this.admin}_selected`,
          res.returnData.result.scaleUserId
        )
        this.setCurMember(res.returnData.result.scaleUserId)
        this.$bridge.pop()
      }
    },
